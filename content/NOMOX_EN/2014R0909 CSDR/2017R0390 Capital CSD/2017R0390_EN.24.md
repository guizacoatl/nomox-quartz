---
surnom_texte_lvl1: CSDR
surnom_texte_lvl2: Capital CSD
texte: 2017R0390
consolidated: True
in_force: true
level: 2
if_level_2_text_level1: 2014R0909
langue: EN
version: 1
last_update: 2026-08-07
---
> [!info]+
> 🔗 Back to [[2017R0390_EN.0|Summary]]. 🇫🇷 French Version: [[2017R0390_FR.24]]. Back to [[2014R0909_EN.0|Summary]] of LVL1. Direct link to [EUR-LEX](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A02017R0390-20170310).
>
> [[2017R0390_EN.23|Article 23 – General requirements for the management of intraday credit risk]] ⬅️ | ➡️ [[2017R0390_EN.25|Article 25 – Potential residual credit exposures]]
>

> [!tip]- References LVL1 <=> LVL2
> Level 1 reference(s): [[2014R0909_EN.59]]

# Article 24 - Credit limits


For the purposes of managing intraday credit risk, and where setting the credit limits to an individual borrowing participant at the group level, a CSD-banking service provider shall comply with all of the following:
### (a)
assess the creditworthiness of the borrowing participant based on a methodology that does not exclusively rely on external opinions;
### (b)
verify the compliance of collateral and other equivalent financial resources provided by a participant to cover intraday credit exposures, with the requirements set out in Articles [[2017R0390_EN.9|9]] and [[2017R0390_EN.15|15]], respectively;
### (c)
set the credit limits to a borrowing participant based on the multiple relationships that the CSD-banking service provider has with the borrowing participant, including where the CSD-banking service provider provides more than one banking-type ancillary service among those referred to in Section C of the Annex to [[2014R0909_EN.0|Regulation (EU) No 909/2014]] to the same participant;
### (d)
take into account the level of qualifying liquid resources in accordance with [[2017R0390_EN.34|Article 34]];
### (e)
review the credit limits to a borrowing participant with the view to ensuring both of the following:
#### (i)
where the creditworthiness of a borrowing participant decreases, that the credit limits are reviewed or reduced;
#### (ii)
where the value of collateral provided by a borrowing participant decreases, that the credit availability is reduced.
### (f)
review the credit lines granted to borrowing participants at least annually based on their actual usage of credit;
### (g)
ensure that the amount of overnight credit exposures is integrated in the usage of the credit limit granted to the participant;
### (h)
ensure that the amount of overnight credit not yet reimbursed is included in the intraday exposures of the next day and is capped by the credit limit.