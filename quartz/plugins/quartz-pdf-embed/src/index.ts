...
/**
 * Plugin PdfEmbed pour Quartz v5
 * Transforme la syntaxe Obsidian ![[fichier.pdf]] en iframe + lien de fallback
 */
export const PdfEmbed: QuartzTransformerPlugin<Options> = (opts?: Options) => {
  ...
  return {
    ...
    textTransform(_ctx, src) {
      // Regex qui capture : ![[fichier.pdf]] ou ![[fichier.pdf|Texte alternatif]]
      return src.toString().replace(
        /!\[\[([^\]]+\.pdf)(?:\|([^\]]*))?\]\]/gi,
        (_match, filename: string, altText: string | undefined) => {
          const label = altText?.trim() || filename
